...
import type { ProduitPublic } from '@/types/public.types';
...
export const ProduitCard: React.FC<ProduitCardProps> = ({ produit, salonTel, salonNom }) => {
  const handleCommander = () => {
    const message = encodeURIComponent(
      `Bonjour ${salonNom},\n\nJe suis intéressé(e) par le produit:\n📦 ${produit.nom}${produit.marque ? ` - ${produit.marque}` : ''}\n💰 Prix: ${produit.prix_actuel.toLocaleString()} FCFA\n\nMerci.`
    );
    
    // Format: enlever les espaces, + et autres caractères
    const telClean = salonTel.replace(/[\s+()-]/g, '');
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow">
      ...
      {/* Contenu */}
      <div className="p-4">
        <h3 className="font-semibold text-lg text-gray-800 mb-1">{produit.nom}</h3>
        ...
        <p className="text-xs text-gray-500 mb-3">
          Stock disponible: {produit.stock_vente}
        </p>
