// src/app/pages/Produits/ProduitCreatePage.tsx
import React, { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { Input } from '@/app/components/ui/input';
import { Button } from '@/app/components/ui/button';
import { Textarea } from '@/app/pages/Produits/components/ui/Textarea';
...
interface VarianteForm {
  id: string; // id local pour React (pas BD)
  reference: string;
...
const defaultVariante = (): VarianteForm => ({
...
interface VarianteFormSectionProps {
  ...
  onChange: (id: string, field: keyof VarianteForm, value: any) => void;
  onChangeAttribut: (id: string, attributId: number, value: string) => void;
...
function VarianteFormSection({
  ...
  const showStockUtilisation = !isReserve && ['utilisation', 'mixte'].includes(variante.type_stock_principal);
  ...
  useEffect(() => {
    ...
    if (quantite > 0) {
      const stockFCFA = stockTotal * variante.taux_change;
      const prixUnitaire = (stockFCFA + frais) / quantite;
      onChange(variante.id, 'prix_achat', prixUnitaire.toFixed(2));
    } else if (stockTotal === 0 && frais === 0) {
      onChange(variante.id, 'prix_achat', '');
    }
  }, [
    variante.prix_achat_stock_total,
    variante.quantite_stock_commande,
    variante.frais_cmb,
    variante.frais_transit,
    variante.frais_bancaires,
    variante.frais_courtier,
    variante.frais_transport_local,
    variante.taux_change,
  ]);

  // Sync stock_reserve avec quantite_stock_commande si type reserve
  useEffect(() => {
    if (variante.type_stock_principal === 'reserve' && variante.quantite_stock_commande) {
      onChange(variante.id, 'stock_reserve', variante.quantite_stock_commande);
    }
  ...
  useEffect(() => {
    ...
    if (devise) {
      onChange(variante.id, 'taux_change', devise.tauxVersFCFA);
    }
  }, [variante.devise_achat]);

  const f = (field: keyof VarianteForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
    onChange(variante.id, field, e.target.value);

  const labelVariante = variante.reference
  ...
  return (
    <div className="border-2 border-blue-200 rounded-xl overflow-hidden">
      ...
      <div
        ...
        onClick={() => onToggleCollapse(variante.id)}
      >
        <div className="flex items-center gap-3">
          <span className="text-sm font-semibold text-blue-800">{labelVariante}</span>
          {variante.prix_vente && (
        ...
        <div className="flex items-center gap-2">
          {total > 1 && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); onRemove(variante.id); }}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="mixte">vente & salon</option>
                  ...
                  <div key={attr.id}>
                    ...
                    {attr.type_valeur === 'liste' ? (
                      <select
                        ...
                        onChange={(e) => onChangeAttribut(variante.id, attr.id, e.target.value)}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={variante.moyen_paiement}
                  onChange={f('moyen_paiement')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  <option value="">Sélectionnez...</option>
              ...
              <div>
                ...
                <select
                  value={variante.devise_achat}
                  onChange={f('devise_achat')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                >
                  {DEVISES.map((d) => (
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                ...
                <div className="flex gap-2 items-center">
                  <Input
                    ...
                    onChange={(e) => onChange(variante.id, 'taux_change', parseFloat(e.target.value) || 1)}
                    min="0"
                    step="0.001"
                    className="w-32"
                  />
                  ...
                  <button
                    ...
                    onClick={() => {
                      const devise = DEVISES.find(d => d.value === variante.devise_achat);
                      if (devise) onChange(variante.id, 'taux_change', devise.tauxVersFCFA);
                    }}
          ...
          <div className="space-y-3">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-4">
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Prix achat stock total
                  </label>
                  <div className="flex gap-1">
                    <Input
                      type="number"
                      value={variante.prix_achat_stock_total}
                      onChange={f('prix_achat_stock_total')}
                ...
                <div>
                  ...
                  <Input type="number" value={variante.quantite_stock_commande} onChange={f('quantite_stock_commande')} min="0" step="1" placeholder="0" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <Input type="number" value={variante.frais_transport_local} onChange={f('frais_transport_local')} min="0" step="0.01" placeholder="0" />
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Prix de vente unitaire (FCFA) <span className="text-red-500">*</span>
                </label>
                <Input
                  type="number"
                  value={variante.prix_vente}
                  onChange={f('prix_vente')}
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                ...
                {variante.quantite_stock_commande && (
                  <div className="flex justify-between items-center pt-2 border-t border-green-200 mt-2">
                    <span className="text-sm text-gray-600">Gain total commande ({variante.quantite_stock_commande} unités)</span>
                    <span className="font-bold text-emerald-600">
                      {formatCurrency(
                        (parseFloat(variante.prix_vente) - parseFloat(variante.prix_achat))
                        * parseFloat(variante.quantite_stock_commande)
                      )} FCFA
                    </span>
                  </div>
                )}
            ...
            {isReserve ? (
              <div className="space-y-4">
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm text-amber-800">
                  ℹ️ Ce produit sera stocké en réserve. Utilisez les transferts pour l'allouer.
                </div>
                <div>
...
export function ProduitCreatePage() {
  ...
  const [variantes, setVariantes]           = useState<VarianteForm[]>([defaultVariante()]);
  ...
  // Charger les attributs selon la catégorie sélectionnée
  useEffect(() => {
    const load = async () => {
      if (!produitData.categorie_id) { setAttributsCategorie([]); return; }
      try {
        const token = tokenStorage.getToken();
        const res   = await fetch(`${import.meta.env.VITE_API_URL}/api/categories/${produitData.categorie_id}`, {
          headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
        });
        const data = await res.json();
  ...
  const handleVarianteChange = (id: string, field: keyof VarianteForm, value: any) => {
    setVariantes(prev => prev.map(v => v.id === id ? { ...v, [field]: value } : v));
  };

  const handleVarianteAttribut = (id: string, attributId: number, value: string) => {
    setVariantes(prev => prev.map(v =>
      v.id === id ? { ...v, attributs: { ...v.attributs, [attributId]: value } } : v
    ));
  };

  const handleAddVariante = () => {
    const nouvelle = defaultVariante();
    // Copier les infos de commande de la première variante pour gagner du temps
  ...
  const handleToggleCollapse = (id: string) => {
    setVariantes(prev => prev.map(v => v.id === id ? { ...v, collapsed: !v.collapsed } : v));
  };
    ...
    try {
      const payload = {
        ...
        variantes: variantes.map(v => {
          ...
          const prixUnitaireDeviseOrigine = stockTotal > 0 && quantite > 0 ? stockTotal / quantite : null;
          ...
          return {
            ...
            prix_achat_devise_origine: prixUnitaireDeviseOrigine,
      ...
      window.close(); // Ferme l'onglet après succès
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="sticky top-0 z-10 bg-white border-b shadow-sm px-6 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{variantes.length} variante{variantes.length > 1 ? 's' : ''}</p>
        </div>
        <div className="flex gap-3">
          ...
          >
            {isSubmitting || isUploadingPhoto ? 'Enregistrement...' : 'Créer le produit'}
          </Button>
        </div>
      </div>

      <form id="produit-form" onSubmit={handleSubmit} className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border p-6 space-y-5">
          <h2 className="text-base font-semibold text-gray-900 bg-gradient-to-r from-blue-50 to-cyan-50 px-4 py-2 rounded-lg border-l-4 border-blue-500">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3 pt-5">
              ...
              />
              <label htmlFor="visible_public" className="text-sm font-medium text-gray-700">
        ...
        <div className="space-y-4">
          ...
          {variantes.length > 1 && (
            <p className="text-xs text-gray-500 text-center">
              💡 Cliquez sur l'en-tête d'une variante pour la réduire
            </p>
          )}
        </div>
