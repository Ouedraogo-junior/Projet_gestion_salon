...
class TransfertStockResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            // Variante + produit parent
            'variante_id'           => $this->variante_id,
            'variante'              => $this->when(
                $this->relationLoaded('variante'),
                [
                    'id'        => $this->variante?->id,
                    'reference' => $this->variante?->reference,
                    'produit'   => $this->when(
                        $this->variante?->relationLoaded('produit'),
                        [
                            'id'  => $this->variante?->produit?->id,
                            'nom' => $this->variante?->produit?->nom,
                        ]
                    ),
                ]
            ),
            ...
            // Mouvements
            'mouvements'             => MouvementStockResource::collection($this->whenLoaded('mouvements')),
            ...
            'temps_attente'          => $this->when(!$this->valide, $this->created_at?->diffForHumans()),
        ];
    }
    ...
    {
        if ($this->valide) {
            return ['label' => 'Validé',     'color' => 'success', 'icon' => 'check-circle'];
        }
